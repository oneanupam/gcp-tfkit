# Template file Data source to render template data.
data "template_file" "startup_script" {
  template = file("${path.module}/bin/vm-startup.sh")
  vars = {
    web_server = "httpd"
  }
}
...
resource "google_compute_instance" "dev_gce01" {
  ...
  boot_disk {
    device_name = join("", [var.gce_name, "-os-disk"])
    auto_delete = true
    mode        = "READ_WRITE"
  ...
  /*
  attached_disk {
    source = google_compute_disk.dev_zonal_pd_01.self_link
    mode   = "READ_WRITE"
  }
  */
